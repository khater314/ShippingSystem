...
namespace BL.Services
{
    ...
    : IBaseService<T, DTO> where T : BaseEntity
    {
        ...
        /* Request Data */
        public virtual async Task<IEnumerable<DTO>> GetAllAsync(CancellationToken ct = default)
        {
            var list = await _repo.GetAllAsync(ct);
            return _mapper.Map<IEnumerable<T>, IEnumerable<DTO>>(list);
        }
        public virtual async Task<DTO> GetByIdAsync(Guid id, CancellationToken ct = default)
        {
        ...
        /* Response Data */
        public virtual async Task AddAsync(DTO entity, CancellationToken ct = default)
        {
        ...
        public virtual async Task<T> AddAndReturnAsync(DTO entity, CancellationToken ct = default)
        {
            var dbEntity = _mapper.Map<DTO, T>(entity);
            dbEntity.CurrentState = 1;
            dbEntity.CreatedBy = await _userService.GetLoggedInUserId();
        ...
        public virtual async Task UpdateAsync(DTO entity, CancellationToken ct = default)
        {
            var dbEntity = _mapper.Map<DTO, T>(entity);
            dbEntity.UpdatedBy = await _userService.GetLoggedInUserId();
            await _repo.UpdateAsync(dbEntity, ct);
        }
        public virtual async Task ChangeStatusAsync(DTO entity, int status = 1, CancellationToken ct = default)
        {
            var dbEntity = _mapper.Map<DTO, T>(entity);
            dbEntity.UpdatedBy = await _userService.GetLoggedInUserId();
